...
export function PassportDemo() {
  ...
  const verify = () => {
    try { verifyGigstarkPassport(verifier, policy, { policyId: policy.id, audience: policy.audience, purpose: policy.purpose, credentialClass: policy.credentialClass, proofCommitment: "opaque-proof:7ac", disclosureCommitment: "studio-tier", scopeNullifier: "studio-scope:42", issuedAt: 1_890, expiresAt: 1_950, nonce: "passport-claim:1" }, now); setMessage("ZeeroStreamPassport accepted the minimum-disclosure claim for this audience."); }
    catch (error) { setMessage(error instanceof Error ? error.message : "PASSPORT_REJECTED"); }
  };
  return <section className={styles.panel}><p className="eyebrow">ZeeroStreamPassport</p><h2>Purpose-bound private access.</h2><p>Only an opaque proof commitment, a scoped replay value, purpose, audience, and expiry are evaluated. No identity document, wallet history, witness, or Athera contract is involved.</p><button onClick={verify}>Verify demo Passport claim</button><p className={styles.notice} role="status">{message}</p></section>;
}
